...
const SingleProject = ({ id, title, isInvitation,onClick }: Props) => {
      ...
      >
        ...
        {isInvitation && open && (
          <div className="flex gap-4 pb-1 pl-2 text-[1rem]">
            <p onClick={()=>onClick({projectId:id,status:'ACCEPTED'})} className="p-0.5 px-1 hover:bg-dark-bg">&#x2713;</p>
            <p onClick={()=>onClick({projectId:id,status:'REJECTED'})} className="p-0.5 hover:bg-dark-bg">&#x2715;</p>
          </div>
        )}
      </div>
    </div>
